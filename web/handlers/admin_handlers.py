...
from core.models import BaseTable, CardsTable, CategoriesTable, CompaniesTable, russian_field_names, tables
...
async def index_get_handler(
    ...
    return TEMPLATES.TemplateResponse(
        ...
        "admin/index.html",
        {
            "tables":
                {
                    "Комании": CompaniesTable.__tablename__,
                    "Карточки": CardsTable.__tablename__,
                    "Категории": CategoriesTable.__tablename__
                }
        }
    )
...
async def get_table_data(
    ...
    """
    table: BaseTable | None = tables.get(data.tablename)
    "Модель таблицы базы данных извлеченная по ее названию"
    if table is not None:
        rows: list[dict[str, Any]] = await get_all_rows_from_table(
                table=table,
                session=session,
...
async def get_table_row_post_handler(
    request: Request,
    data: GetTableRowModel,
    session: AsyncSession = Depends(async_session_generator)
):
    """
    ...
    if table is not None:
        row_info: dict[str, Any] = await get_full_row_for_admin_by_id(
            row_id=data.id,
            table=table,
            session=session,
            queue_name=FASTAPI_DATABASE_QUERIES_QUEUE_NAME
        ...
        return TEMPLATES.TemplateResponse(
            request,
            "admin/modal_save.html",
            {
                "row_id": data.id,
                "row_info": row_info,
                "descriptions": russian_field_names,
                "columns": row_info.keys()
            }
        )


@admin_rt.post("/save_row")
async def save_row_post_handler(
    data: SaveRowModel,
    session: AsyncSession = Depends(async_session_generator)
):
    """
    ...
    Args:
        data (SaveRowModel): Модель с именем таблицы, идентификатором строки и словарём новых данных.
        session (AsyncSession): Асинхронная сессия SQLAlchemy для работы с БД.
    ...
    "Модель таблицы базы данных извлеченная по ее названию"
    if table is not None:
        normalized_data: dict[str, Any] = map_columns_to_table_types(
            table=table,
            data=data.data
        )

        update_res: bool | str = await update_row_by_id(
            row_id=data.id,
            table=table,
...
async def create_row_get_handler(
    request: Request,
    data: CreateRowGetModalModel
):
    """
...
async def create_row_post_handler(
    data: CreateRowModel,
    session: AsyncSession = Depends(async_session_generator)
):
    """
    ...
    if table is not None:
        ...
        "Приведенные к нужным типам данные, отражающие новую запись в бд"

        new_id: int | str = await create_row(
            table=table,
            data=normalized_data,
            session=session,
            queue_name=FASTAPI_DATABASE_QUERIES_QUEUE_NAME
...
async def delete_row_post_handler(
    data: DeleteRowModel,
    session: AsyncSession = Depends(async_session_generator)
):
    """
    ...
    "Модель таблицы базы данных извлеченная по ее названию"
    if table is not None:
        del_res: bool = await delete_row(data.id, table, session, FASTAPI_DATABASE_QUERIES_QUEUE_NAME)
        "Результат удаление столбца"
        if del_res is True:
            return JSONResponse(
                {
                    "ok": True
